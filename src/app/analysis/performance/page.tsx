...
export default function PerformancePage() {
  ...
  useEffect(() => {
    if (!driveId) {
      // Eğer driveId yoksa, kullanıcıyı yükleme sayfasına yönlendir
      router.replace('/upload');
    } else {
      // DriveId varsa, localStorage'dan veriyi al
      const savedData = getDriveData(driveId);
      if (savedData) {
        setDriveData(savedData);
      } else {
        // Eğer veri bulunamazsa, kullanıcıyı yükleme sayfasına yönlendir
        router.replace('/upload');
      }
    }
  }, [searchParams, router]);
